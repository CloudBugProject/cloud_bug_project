...
def create_app() -> Flask:
    ...
    def upload():
        ...
        try:
            ...
            _insert_log(app.config["DATABASE_PATH"], email, dt.datetime.utcnow(), malicious_ratio, total_count, malicious_count)
...
def _insert_log(db_path: str, email: str, uploaded_at: dt.datetime, ratio: float, total: int, mal: int) -> None:
    with sqlite3.connect(db_path) as conn:
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO uploads (email, uploaded_at, malicious_ratio, total_rows, malicious_rows) VALUES (?, ?, ?, ?, ?)",
...
sys.modules["__main__"].safe_log1p = safe_log1p
...
def _load_model(model_path: str):
    ...
    try:
        ...
        if os.path.exists(model_path):
            ...
            # ✅ safe_log1p를 전역 등록
            import builtins
            builtins.safe_log1p = safe_log1p
